		...
		{#each data.countries as country}
			...
			>
				...
				<div class="card-body w-full md:items-start justify-center gap-4 p-8 pt-0 md:pt-8 md:pl-0">
					...
					{#if country.national_referent.email}
						...
						<div class="w-full flex flex-wrap">
							<p class="text-lg text-center">
								{country.national_referent.name || ''},
								<a
									href="mailto:{country.national_referent.email || ''}"
									class="btn-link link-secondary font-semibold"
								>
									{country.national_referent.email || ''}
								</a>
							</p>
						</div>
					{/if}

					{#if country.scientific_coordinator.email}
						...
						<div class="w-full flex flex-wrap">
							<p class="text-lg text-center">
								{country.scientific_coordinator.name},
								<a
									href="mailto:{country.scientific_coordinator.email}"
									class="btn-link link-secondary font-semibold"
								>
									{country.scientific_coordinator.email}
								</a>
							</p>
						</div>
					{/if}
